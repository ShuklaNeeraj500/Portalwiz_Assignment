...
const Level_comp = ()=>{
    ...
    const Column = [
      { title: 'ID', field: 'level_id',sorting:false, searchable:false },
      { title: 'Level', field: 'level_name' },
      { title: 'Description', field: 'level_desc' },
      {
        title: 'Active', field: 'level_status',
...
// ------------------------Fetching Level table's data-------------------------------------
    useEffect(() => {
      ...
      const obj = { uname:state.uname};
        axios.post("http://sandbox.portalwiz.com/php/levels.php", obj)
        .then( (response) => {
...
// ------------------------Deleting Level table's data-------------------------------------
      ...
      const onDelete =(id) =>{
        ...
        axios.post("http://sandbox.portalwiz.com/php/delete_level.php", delobj)
       ...
       <Heading name="Levels"/>
        ...
        <div className="container">
            ...
            <div className="col-11 mx-auto d-flex justify-content-end">
                <Link to="/add-level" className="btn btn-primary btn_link">Add Level&nbsp;<Add /></Link>
            </div>
            </div>
                <div className="row ">
                   ...
                   <MuiThemeProvider theme={theme}>
                        <MaterialTable
                          ...
                          actions={[
                            ...
                            //   icon : View,
                            //   tooltip: 'View Level',
                            //   onClick: (event, rowData) => history.push("/view-level/" + rowData[0].level_id),
                          ...
                          //----------------------Edit Icon--------------- 
                            rowData => ({
                              icon : Edit,
                              tooltip: 'Edit Level',
                              onClick: (event, rowData) => history.push("/edit-level/" + rowData[0].level_id),
                              hidden: rowData.length > 1
                            }),
                           //----------------------Delete Icon--------------- 
                            rowData => ({
                          ...
                          onRowClick={
                              (event,rowData)=>{
                                console.log(rowData)
                                history.push("/view-level/" + rowData.level_id)
                              }
                          }
                        />
                        </MuiThemeProvider>
